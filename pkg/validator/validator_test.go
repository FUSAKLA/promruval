...
var testCases = []struct {
	...
	{name: "ruleExprDoesNotUseAnyLabels", validator: expressionUseOnlyWhitelistedLabelsForMetric{allowedLabels: []string{"label_app"}, metric: "kube_pod_labels"}, rule: rulefmt.Rule{Expr: "kube_pod_labels"}, expectedErrors: 0},
	{name: "ruleExprDoesUseForbiddenLabelInSelector", validator: expressionUseOnlyWhitelistedLabelsForMetric{allowedLabels: []string{}, metric: "kube_pod_labels"}, rule: rulefmt.Rule{Expr: "kube_pod_labels{app='foo'}"}, expectedErrors: 1},
	{name: "ruleExprDoesUseForbiddenLabelInBy", validator: expressionUseOnlyWhitelistedLabelsForMetric{allowedLabels: []string{}, metric: "kube_pod_labels"}, rule: rulefmt.Rule{Expr: "sum(kube_pod_labels) by (app)"}, expectedErrors: 1},
	{name: "ruleExprDoesUseForbiddenLabelInOn", validator: expressionUseOnlyWhitelistedLabelsForMetric{allowedLabels: []string{}, metric: "kube_pod_labels"}, rule: rulefmt.Rule{Expr: "kube_pod_labels * on(app) up"}, expectedErrors: 1},
	{name: "ruleExprDoesUseForbiddenLabelInGroup", validator: expressionUseOnlyWhitelistedLabelsForMetric{allowedLabels: []string{}, metric: "kube_pod_labels"}, rule: rulefmt.Rule{Expr: "group(kube_pod_labels) by (label_app)"}, expectedErrors: 1},
	{name: "ruleExprDoesUseForbiddenLabelInBinaryExprWithLabelTransfer", validator: expressionUseOnlyWhitelistedLabelsForMetric{allowedLabels: []string{}, metric: "kube_pod_labels"}, rule: rulefmt.Rule{Expr: "kube_pod_labels * on(app) group_left(foo) up"}, expectedErrors: 2},
	{name: "ruleExprDoesUseForbiddenLabelInBinaryExprWithLabelTransfer", validator: expressionUseOnlyWhitelistedLabelsForMetric{allowedLabels: []string{}, metric: "kube_pod_labels"}, rule: rulefmt.Rule{Expr: "kube_pod_labels * on(app) group_right(foo) up"}, expectedErrors: 2},
	{name: "ruleExprDoesUseForbiddenLabelInBinaryExprWithLabelTransfer", validator: expressionUseOnlyWhitelistedLabelsForMetric{allowedLabels: []string{"app"}, metric: "kube_pod_labels"}, rule: rulefmt.Rule{Expr: "kube_pod_labels * on(app) group_left(foo) up"}, expectedErrors: 1},
	{name: "ruleExprDoesUseForbiddenLabelInBinaryExprWithLabelTransfer", validator: expressionUseOnlyWhitelistedLabelsForMetric{allowedLabels: []string{"app"}, metric: "kube_pod_labels"}, rule: rulefmt.Rule{Expr: "kube_pod_labels * on(app) group_right(foo) up"}, expectedErrors: 1},
